...
#define LENGTH 500 // Length in bytes
...
class ImageSubscriber : public rclcpp::Node
{
	...
	private:
		...
		void onImageMsg(const sensor_msgs::msg::Image::SharedPtr msg) {
			...
			sleep(1);
			uint32_t *read_buff = (uint32_t *)malloc(LENGTH);
			uint32_t ret = 0;
			pmem.gather(read_buff, P_OFFSET, LENGTH);

			for (int i = 0; i < LENGTH / sizeof(uint32_t); i++)
			{
				std::cout << "W: " << u_buff[i] << " R: " << read_buff[i] << std::endl;
			}

			// Convert the data to a cv::Mat
			cv::Mat thresholded_img(cam_img.rows, cam_img.cols, CV_8UC1);
			for (int row = 0; row < 480; row++)
			{
				for (int col = 0; col < 640; col++)
				{
					thresholded_img.at<uint8_t>(row, col) = (uint8_t)read_buff[row*640 + col];
				}
			}

			// Convert the image to a ROS message
			sensor_msgs::msg::Image::SharedPtr thresholded_message = cv_bridge::CvImage(std_msgs::msg::Header(), "mono8", thresholded_img).toImageMsg();
	  		threshold_publisher->publish(*thresholded_message.get());


			// Gray scale image to ROS message
